...
describe('Form', () => {
  ...
  it('should not render password confirmation field when showPasswordConfirmation is false', () => {
    ...
    expect(screen.queryByPlaceholderText('Подтвердите пароль')).not.toBeInTheDocument();
  });
  ...
  it('should submit form with valid data', async () => {
    ...
    await waitFor(() => {
      expect(mockHandleClick).toHaveBeenCalledWith('test@example.com', 'password123', undefined);
    });
  });
  ...
  it('should submit form with password confirmation', async () => {
    ...
    await waitFor(() => {
      expect(mockHandleClick).toHaveBeenCalledWith('test@example.com', 'password123', 'password123');
    });
  });
