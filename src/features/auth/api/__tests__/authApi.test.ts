...
describe('authApi', () => {
  ...
  describe('signIn', () => {
    ...
    it('should throw error when sign in fails', async () => {
      ...
      await expect(signIn(credentials)).rejects.toThrow('Ошибка входа: Invalid credentials');
    });
    ...
    it('should handle unknown error', async () => {
      ...
      await expect(signIn(credentials)).rejects.toThrow('Ошибка входа: Неизвестная ошибка');
    });
  });
  ...
  describe('signUp', () => {
    ...
    it('should throw error when sign up fails', async () => {
      ...
      await expect(signUp(credentials)).rejects.toThrow('Ошибка регистрации: Email already in use');
    });
    ...
    it('should handle unknown error', async () => {
      ...
      await expect(signUp(credentials)).rejects.toThrow('Ошибка регистрации: Неизвестная ошибка');
    });
  });
  ...
  describe('logout', () => {
    ...
    it('should handle unknown error', async () => {
      ...
      await expect(logout()).rejects.toThrow('Ошибка выхода: Неизвестная ошибка');
    });
  });
});