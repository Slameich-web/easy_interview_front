...
export const useProgress = (topicId: string) => {
  ...
  const loadProgress = useCallback(async () => {
    ...
    console.log("Загружаем прогресс для пользователя:", userId, "тема:", topicId);
    setIsLoading(true);
    setError(null);
  ...
  const submitAnswer = useCallback(async (
    ...
    try {
      ...
      const correctAnswers = Object.values(updatedAnswers).filter(a => a.isCorrect).length;
      const totalAnswers = Object.keys(updatedAnswers).length;
      const newScore = totalAnswers > 0 ? Math.round((correctAnswers / totalAnswers) * 100) : 0;

      await upsertProgress(userId, topicId, userData.studentId, newAnswer, newScore);
  ...
  const getAnswerForQuestion = useCallback((questionId: string): Answer | null => {
