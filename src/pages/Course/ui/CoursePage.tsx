...
const CoursePage = () => {
  ...
  if (!currentPlan) {
    return (
      <NotFoundContainer>
        <NotFoundTitle variant="h4">Курс не найден</NotFoundTitle>
  ...
  const totalQuestions = topics.reduce(
    (sum, topic) => sum + topic.questionsCount,
    0
  );
  const heroStats =
    topics.length > 0
  ...
  return (
    <>
      <PageHeader title={currentPlan.name} icon="📖" backTo="/study-plans" />
          ...
          <Grid container spacing={{ xs: 3, md: 4 }}>
            {topics.map((topic) => (
              <Grid key={topic.id} size={{ xs: 12, sm: 6, lg: 4 }}>
                <TopicCard topic={topic} onSelect={handleSelectTopic} />
              </Grid>
            ))}
          </Grid>
        )}
